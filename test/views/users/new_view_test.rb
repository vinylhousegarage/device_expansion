...
class UsersNewViewTest < ActionView::TestCase
  ...
  test 'renders the new user invitation form' do
    ...
    assert_select 'form[action=?][method=?]', login_form_session_path(users(:first_poster)), 'post' do
      assert_select 'button', '投稿者１さんを招待する'
    end
    assert_select 'form[action=?][method=?]', login_form_session_path(users(:second_poster)), 'post' do
